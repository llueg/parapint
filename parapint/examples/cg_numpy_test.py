...
A = BlockMatrix(4, 4)
A.set_block(0, 0, coo_matrix(np.array([[-4, 1],
                                        [0, 1]], dtype=np.double)))
A.set_block(1, 1, coo_matrix(np.array([[1, 0],
                                        [0, 1]], dtype=np.double)))
A.set_block(2, 2, coo_matrix(np.array([[1, 0],
                                        [1, 1]], dtype=np.double)))
A.set_block(3, 3, coo_matrix(np.array([[-2, 0],
                                        [0, 0]], dtype=np.double)))
...
sc_solver = parapint.linalg.ImplicitSchurComplementLinearSolver(subproblem_solvers={ndx: ScipyInterface(compute_inertia=True) for ndx in range(3)},
                                                        schur_complement_solver=ScipyInterface(compute_inertia=True))
sc_solver.do_symbolic_factorization(A + A_upper)
sc_solver.do_numeric_factorization(A + A_upper)
x2 = sc_solver.do_back_solve(rhs)
print(f'condidtion A: {np.linalg.cond(A.toarray())}')
print(f'actual solution {x1}')
print(f'computed solution {x2.flatten()}')
...
print(np.allclose(x1, x2.flatten()))
print(f'actual inertia: {inertia2}')
print(f'computed inertia: {inertia1}')
