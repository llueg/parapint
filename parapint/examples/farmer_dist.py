...
import parapint.interfaces.distributed_regularization


"""
To run this example:

mpirun -np 3 python -m mpi4py farmer_dist.py
"""
...
def create_scenario(farmer: Farmer, scenario: str):
    ...
    def bilinear_constraint_rule(m, i, j):
        if i == j:
            pe.Constraint.Skip
        return m.devoted_acreage[i] * m.devoted_acreage[j] <= 100
    m.bilinear_constraint = pe.Constraint(m.crops,m.crops, rule=bilinear_constraint_rule)
...
class DistProblem(parapint.interfaces.distributed_regularization.MPIDistStochasticSchurComplementInteriorPointInterface):
    def __init__(self,
                 farmer: Farmer):
        self.farmer: Farmer = farmer
        first_stage_var_ids = [('devoted_acreage', i) for i in self.farmer.crops]
        super(DistProblem, self).__init__(scenarios=self.farmer.scenarios,
                                      nonanticipative_var_identifiers=first_stage_var_ids,
                                      comm=comm)
...
def main(farmer: Farmer, subproblem_solver_class, subproblem_solver_options):
    ...
    interface = DistProblem(farmer=farmer)
