...
class MPIImplicitSchurComplementLinearSolver(LinearSolverInterface):
    ...
    def do_back_solve(self, rhs, timer=None):
        ...
        coupling = self.do_sc_back_solve_scipy_minimize(schur_complement_rhs)
        coupling, info = sps.linalg.cg(self._sc, schur_complement_rhs, rtol=1e-7)
    ...
    def do_sc_back_solve_scipy_minimize(self, sc_rhs, timer=None):
        
        x0 = np.ones_like(sc_rhs)

        def func(x):
            y = self._sc @ x
            r = y - sc_rhs
            res = 0.5 * np.dot(x, y) - np.dot(x, sc_rhs)
            return (res, r)

        #sol = scipy.optimize.minimize(func, x0, jac=True, method='L-BFGS-B', options={'maxcor': 20, 'maxiter': 1000})
        sol = sco.minimize(func, x0, jac=True, method='L-BFGS-B', options={'maxiter': 1000}, tol=1e-5)
        print(f'PCG #iter: {sol.nit}, success {sol.success}, message {sol.message}')
        #print(sol)
        return sol.x
