...
class ImplicitSchurComplementLinearSolver(LinearSolverInterface):
    """
    ...
    def do_back_solve(self, rhs):
        ...
        # apply pcg to schur complement
        coupling, info = self.pcg_schur_backsolve(schur_complement_rhs)
        if info != 0:
    ...
    def get_inertia(self):
        ...
        # only stochastic schur should use pcg
        # _pos, _neg, _zero = 
        # num_pos += _pos
        # num_neg += _neg
        # num_zero += _zero
    ...
    def pcg_schur_backsolve(self, schur_complement_rhs):
        D = self.block_matrix.get_block(self.block_dim - 1, self.block_dim - 1).tocoo()

        def schur_matvec_serial(u):
            res = np.zeros_like(u)
            for ndx in range(self.block_dim - 1):
        ...
        lin_op = LinearOperator(shape=D.shape, matvec=schur_matvec_serial, dtype=float)

        iters = 0
        def nonlocal_iterate(arr):
            nonlocal iters
            iters+=1
        coupling, info = cg(A=lin_op, b=schur_complement_rhs, tol=1e-010, callback=nonlocal_iterate)
...
#def matvec_pynumero(u):
    # transform u to block vector
    # get i based on rank
    #v_i = A_i * u
    #x_i = subsolver[i].backsolve(v_i)
    #y_i = A.T * x_i
    #t = allreduce(y_i, sum)
    #res = del_H * I * u  + t
    #return res.to_numpy()

#lin_op = LinearOperator(shape=(n,n), matvec=matvec_pynumero)

#result = cg(lin_op, schur_rhs)